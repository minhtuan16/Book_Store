...
public class ShoppingCart {
	...
	public Double getTotalPrice() {
		...
		for(CartItem item : this.items) {
			sum = sum + item.getProduct().getPrice() * item.getQuantity() - item.getProduct().getDiscount().getDiscount() * item.getProduct().getPrice() * item.getQuantity() / 100;
		}
		return sum;
	}
